...
- **The subscription stalls:** it handles no later events until you act. Other consumers keep running. Restart the app to retry the event once more, or skip it; see [poison event](/operations/poison-event/).

The default retries stop after about 30 seconds. For a service with longer outages, raise `HandlerRetries`, or retry in the handler with a resilience library such as Polly. The runner settings apply to every consumer; [configuration](/reference/configuration/) lists them.
