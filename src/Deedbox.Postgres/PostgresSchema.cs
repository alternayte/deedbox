...
/// <summary>The Deedbox schema for Postgres, as SQL for DBAs and migration tools.</summary>
public static class PostgresSchema
{
    /// <summary>The schema version this build of Deedbox needs.</summary>
    public static int LatestVersion => PostgresProvider.AllMigrations[^1].Version;

    /// <summary>
    /// The SQL for every migration after <paramref name="fromVersion"/>. Pass it to
    /// <c>migrationBuilder.Sql(...)</c> in an EF Core migration, or to DbUp or Flyway.
...
=>
        SchemaScript.Render(PostgresProvider.AllMigrations, SchemaName.Validate(schema), fromVersion);
}
